...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // ✅ PERTAHANKAN hanya ini:
    if (empty($errors)) {
        $stmt = $conn->prepare("SELECT id_anggota FROM anggota WHERE kode_anggota = ? AND id_anggota != ?");
        $stmt->bind_param("si", $kode_anggota, $id_anggota);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result->num_rows > 0) {
    ...
    if (count($errors) == 0) {
        $stmt = $conn->prepare("UPDATE anggota SET kode_anggota=?, nama=?, email=?, telepon=?, alamat=?, tanggal_lahir=?, jenis_kelamin=?, pekerjaan=?, tanggal_daftar=?, foto=?, status=? WHERE id_anggota=?");

    $stmt->bind_param("sssssssssssi", 
        $kode_anggota, $nama, $email, $telepon, $alamat,
        $tanggal_lahir, $jenis_kelamin, $pekerjaan,
        $tanggal_daftar, $foto, $status, $id_anggota
    );
        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <!-- ✅ Tambahkan id di action form -->
                    <form method="POST" action="edit.php?id=<?php echo $id_anggota; ?>" enctype="multipart/form-data">
                        <!-- Kode Anggota & Nama -->
                        <div class="row">
